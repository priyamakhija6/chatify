...
const resolvers = {
    ...
    Mutation: {
        ...
        signinUser: async (_, {userSignin}) => {
            ...
            if (!user) throw new AuthenticationError("User Doesn't Exist! Please Sign Up.")
            const doMatch = await bcrypt.compare(userSignin.password, user.password)
            if (!doMatch) throw new AuthenticationError("Invalid Email and/or Password! Try Again.")
            const token = jwt.sign({userId: user.id}, process.env.JWT_SECRET)
            return {token}
        },
        createMessage: async (_, {receiverId, text}, {userId}) => {
            if (!userId) throw new ForbiddenError("Please Log In.")
